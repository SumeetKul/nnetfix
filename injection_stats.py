...
n_injections = 2 # Number of injections
...
for i in range(n_injections):
	...
	if i % 10 == 0:
		print("{} out of {} injection frames NNETFIXED successfully!".format(i*10,n_injections))

np.savetxt(os.path.abspath("injections/{}/Inj_snr.csv".format(data_dir)),SNR_array, fmt='%1.3f',delimiter=',',header="Index, Original, Gated, Reconstructed")
np.savetxt(os.path.abspath("injections/{}/Inj_chisq.csv".format(data_dir)),chisq_array,fmt='%1.3f',delimiter=',',header="Index, Original, Gated, Reconstructed")
np.savetxt(os.path.abspath("injections/{}/Inj_params.csv".format(data_dir)),inj_param_array,fmt='%1.3f',delimiter=',',header="Index, mass1, mass2, RA, DEC, Polarization, SNR")
